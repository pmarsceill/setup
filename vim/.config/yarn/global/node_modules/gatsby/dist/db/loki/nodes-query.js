...
async function runQuery(args) {
  if (args.nodeTypeNames.length > 1) {
    const nodes = args.nodeTypeNames.reduce((acc, typeName) => acc.concat(getNodesByType(typeName)), []);
  ...
  if (sortFields) {
    ...
    if (sortFieldsHaveArray) {
      const sortFieldAccessors = sortFields.map(([field, _]) => v => getValueAt(v, field));
