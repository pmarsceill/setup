...
function isEqId(firstOnly, siftArgs) {
  return firstOnly && siftArgs.length > 0 && siftArgs[0].id && Object.keys(siftArgs[0].id).length === 1 && Object.keys(siftArgs[0].id)[0] === `$eq`;
}
...
const runSift = args => {
  ...
  if (nodeTypeNames.length > 1) {
    nodes = nodeTypeNames.reduce((acc, typeName) => acc.concat(getNodesAndResolvedNodes(typeName)), []);
  } else {
