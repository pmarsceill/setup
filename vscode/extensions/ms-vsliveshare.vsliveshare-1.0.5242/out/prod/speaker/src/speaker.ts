...
let isPlaying = false;
let resolveInterruptPromise: (continuePlaying: boolean) => void;
let interruptPromise: Promise<boolean> = new Promise((resolve, reject) => resolveInterruptPromise = resolve);

let receivedAudio: Buffer[] = [];
let receivedAudioIdx = 0;
let receivedAudioByteLength = 0;
function receiveAudioData(audioBuffer: Buffer) {
    if (!isPlaying) {
...
async function play(): Promise<void> {
    ...
    while (continuePlaying && totalBytesPlayed < receivedAudioByteLength) {
        ...
        const continuePlayingPromise = new Promise<boolean>((resolve, reject) => setTimeout(() => resolve(true), msPerChunk));
...
function write(bytes: number): number {
...
async function waitUntilDonePlaying(): Promise<void> {
    const timeUntilDonePlaying = bufferedUntil - (new Date()).getTime();
    if (timeUntilDonePlaying > 0) {
        await new Promise<void>((resolve, reject) => setTimeout(() => resolve(), timeUntilDonePlaying));
    }
}
